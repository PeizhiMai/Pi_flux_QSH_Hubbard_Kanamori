@testitem "HundSpinFlipHST construction and smoke update" begin
    ...
    hst = HundSpinFlipHST(model_geometry = model_geometry, JH = 0.25, β = 0.2, Δτ = 0.1, rng = rng)
    initialize!(fpi_up, fpi_dn, hst)

    @test fpi_up.V == fill(0.125, 4, 2)
    @test fpi_dn.V == fill(0.125, 4, 2)
